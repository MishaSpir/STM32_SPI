...
// void NRF24::Timer_delay_10us(uint32_t data)
// {
// uint32_t time = (timer_get_counter(TIM2) + data);
...
// void NRF24::Timer_delay_1ms(uint32_t data)
// {
// uint32_t time = (timer_get_counter(TIM2) + data*100);
...
void NRF24::send_SPI (uint8_t data)
{
    spi_send(SPI1, data);
...
uint8_t NRF24::Transfer_SPI(uint8_t data)
{
  uint8_t read_data;
  send_SPI(data);
  read_data = read_SPI();
...
uint8_t NRF24::read_register(uint8_t reg, uint8_t* buf, uint8_t len)
{
  ...
  csn(LOW);
  status = Transfer_SPI( R_REGISTER | ( REGISTER_MASK & reg ) );
  while ( len-- )
...
{
  csn(LOW);
  Transfer_SPI( R_REGISTER | ( REGISTER_MASK & reg ) );
  uint8_t result = Transfer_SPI(0xff);
...
uint8_t NRF24::write_register(uint8_t reg, const uint8_t* buf, uint8_t len)
{
  ...
  csn(LOW);
  status = Transfer_SPI( W_REGISTER | ( REGISTER_MASK & reg ) );
  while ( len-- )
    Transfer_SPI(*buf++);
...
NRF24::NRF24(uint16_t _cepin, uint32_t _ceport, uint16_t _cspin,uint32_t _csport):
...
bool NRF24::setChannel(uint8_t channel)
{
  ...
  // Verify our result
  if ( read_register(RF_CH) == channel )
  {
    result = true;
  }
...
bool NRF24::begin(void)
{
  ...
  // hardware.
  result &= setDataRate( RF24_1MBPS ) ;

  // Initialize CRC and request 2-byte (16bit) CRC
  result &= setCRCLength( RF24_CRC_16 ) ;
...
bool NRF24::write( const void* buf, uint8_t len )
{
  ...
  uint8_t observe_tx;
  uint8_t status;
  uint32_t sent_at = timer_get_counter(TIM2);
  const uint32_t timeout = 500*100; //ms to wait for timeout
  do
  {
    status = read_register(OBSERVE_TX,&observe_tx,1);
  }
  while( ! ( status & ( _BV(TX_DS) | _BV(MAX_RT) ) ) && ( timer_get_counter(TIM2) - sent_at < timeout ) );
  ...
  // Power down
  // powerDown(); // <<<--- Закоментировал, иначе посылал только одну посылку и замолкал.
...
void NRF24::startWrite( const void* buf, uint8_t len )
{
...
bool NRF24::read( void* buf, uint8_t len )
{
  // Fetch the payload
  read_payload( buf, len );
  
  write_register(STATUS, (1 << RX_DR) | (1 << MAX_RT) | (1 << TX_DS)); // <<<--- Добавил, иначе принимал только одну посылку
...
void NRF24::writeAckPayload(uint8_t pipe, const void* buf, uint8_t len)
{
  ...
  csn(LOW);
  Transfer_SPI( W_ACK_PAYLOAD | ( pipe & 0x7 ) );
  const uint8_t max_payload_size = 32;
  uint8_t data_len = std::min(len,max_payload_size);
  while ( data_len-- )
    Transfer_SPI(*current++);
...
bool NRF24::setCRCLength(rf24_crclength_e length)
{
  bool result = false;
  ...
  // Verify our result
  if ( read_register(CONFIG) == config )
  {
    result = true;
  }
