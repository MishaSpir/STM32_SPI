# переписываем библиотеку(RF24-master) c Arduino нас STM32
# 1 Этап
## write_register()
**- на Arduino:**
...
Кстати, немного пояснений. Передача происходит парами: сначала номер регистра, затем его значение.
 Когда хотим записать данные в регистр, к его адресу прибавляется 0x20, когда прочитать, то 0x00, то есть ничего.

# 2 Этап
**Приёем и передача на ардуино:**

![alt text](pictures/send_recieve_ard.png)


## Ситуация на 24.11
Получилось принять ОДИН раз. Приём корректный (но одноразвый), данные можно посмотерть по ComPort ( по UART от stm-ки). Не знаю почему так происходит... Приёмник принимает один раз, а потом игнорирует передатчик...Буду разбираться.
